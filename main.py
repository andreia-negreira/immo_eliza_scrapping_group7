...
def get_data(url):
    ...
    if req.status_code == 200: # Checking if the link is still valid
        ...
        try:
            property_dict["postal_code"] = Raw_data_InDict["property"]['location']['postalCode']
        except KeyError:
            property_dict["postal_code"] = 0
        except TypeError:
            property_dict["postal_code"] = 0    
...
links = get_links(1) # Set the number according to how many pages you want to search thru each of properties set in get_links function's propertiesToSearch variable.
end = time.time()
